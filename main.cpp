 ...
 * @author H0ly
 */
int main() { /**< Объявляет главную функцию программы. */
    debugLog << "[" << getCurrentTime() << "] Программа запущена\n"; /**< Логирует запуск программы. */

    try { /**< Начинает блок обработки исключений для всего игрового процесса. */
        ...
        while (true) { /**< Запускает цикл обработки главного меню. */
            int choice = menu.display(); /**< Отображает главное меню и получает выбор пользователя. */
            if (choice == 0) { /**< Проверяет, выбран ли пункт "Start Game". */
            ...
            } else if (choice == 1) { /**< Проверяет, выбран ли пункт "Select Difficulty". */
            ...
            } else if (choice == 2) { /**< Проверяет, выбран ли пункт "Exit". */
